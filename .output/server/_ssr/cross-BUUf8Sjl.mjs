...
//#region node_modules/.nitro/vite/services/ssr/assets/cross-BUUf8Sjl.js
...
function normalizeAnalysis(raw, validCodes) {
	...
	return {
		...
		actionItems: asArr$1(obj.actionItems ?? obj.action_items).map((item) => {
			const a = item && typeof item === "object" ? item : {};
			return {
				assignee: str$1(a.assignee),
				deadline: str$1(a.deadline),
				task: str$1(a.task),
				segmentId: keepCode(str$1(a.segment_id ?? a.segmentId))
			};
		}).filter((a) => a.task)
	};
}
